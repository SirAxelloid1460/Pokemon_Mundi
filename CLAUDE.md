...
- **PENDIENTE (formas regionales)**: en las secciones regionales, si una especie tiene forma regional debe mostrarse **solo esa forma** (no la base + la regional). En la Nacional, la ficha será más rica con un selector para ver **todas las formas** de cada especie. Falta dataset de formas (Alola/Galar/Hisui/Paldea + alternas) en `Pokemon.json`; los sprites de forma usan `{dex}_{genero}_{region}.png`.
